...
#                   bunx eslint .; bun run build;
#                   bun test tests/ ./src ./scripts supabase/functions/_shared
...
job_frontend() {
  ...
  run_step frontend "Typecheck strict boundary (ADR 015)" "$ROOT" bun run typecheck:strict || failed+="typecheck:strict "
  ...
  run_step frontend "Tests (bun test tests/ ./src ./scripts supabase/functions/_shared)" "$ROOT" bun test tests/ ./src ./scripts supabase/functions/_shared || failed+="tests "
...
generate_shared_typecheck_barrel() {
  ...
  rm -f "$barrel"
  {
    ...
    for f in "$shared"/*.ts; do
      base="$(basename "$f")"
      # See the same skip in ci.yml — these two loops must stay identical.
